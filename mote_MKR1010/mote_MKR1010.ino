...
#define HOSTNAME "wm20" // must be unique for all motes on network
#define TOPIC_OUTGOING "wm20/outgoing" // measurements published on this topic
#define TOPIC_SHADOW_UPDATE "$aws/things/Arduino_WM20/shadow/update" // used to report the updated shadow state
#define TOPIC_SHADOW_OUT "$aws/things/Arduino_WM20/shadow/get" // used for Arduino to ask AWS to publish shadow state, which is then published on:...
#define TOPIC_SHADOW_IN "$aws/things/Arduino_WM20/shadow/get/accepted" // ...this one
#define CERT WM20_CERTIFICATE

// Measurement frequency in Hz and number of measurements to be cached by sensor:
#define FREQ 12.5
#define NUM_MEAS 6
#define MS_DT -4 // add this many milliseconds to the wait period (depends on NUM_MEAS, tune to get exactly 12.5 Hz measurements)

// Time to sleep or take measurements before re-checking desired state (in minutes):
#define WAKE_CHECK_PERIOD 60 * 60000
#define SLEEP_CHECK_PERIOD 20 * 60000
#define ONE_HOUR 60 * 60000UL

// Battery settings:
#define MIN_BATT_VOLTAGE 3.5   // deepSleep if voltage falls below this
// With solar charger:
#define BATT_PIN A0
...
unsigned long lastRTCSetTime, lastMSCalibMillis;
...
void setup()
{
  ...
  Serial.println(F(" : 2023/04/07 Build - Pressure, WiFi"));
  Serial.print(F("SSID: "));
  Serial.println(SSID);
  ...
  // Deep sleep if battery voltage is low:
  checkBattery();
  ...
  if (!verifyTime()) {
    // Failed, use the Wifi time instead:
    lastRTCSetTime = setRTCwithRadio();
  }
  Watchdog.reset();
...
void loop() {
  if (sleeping == 1) {
    ...
    Serial.println(F("Shutting down ECC chip..."));
    ECCX08.end();
    ...
    LowPower.deepSleep(SLEEP_CHECK_PERIOD);
    ...
    if (millis() > lastResetMillis + WAKE_CHECK_PERIOD) {
    ...
    if (millis() > lastMSCalibMillis + 120000 && milliseconds > 970 && milliseconds < 990) { //the below code waits for a new second to rollover, so only run it when it's about to happen anyway otherwise just wasting time
      lastMSCalibMillis = calibMS();
    ...
    publishReadings(Pa, Ta, Pb, Tb, Pc, Tc);
  }
}
...
//=============================================================================================
/*
   Set the DS3231 by getting the time from the WiFi radio.
*/
unsigned long setRTCwithRadio() {
  Serial.println(F("Synchronizing RTC with WiFi time"));
  unsigned long initTime = WiFi.getTime();
  unsigned long start = millis();
  while (WiFi.getTime() == initTime && millis() < start + 5000) {}
...
/*
   ...
   Since MQTTClient is set to listen to the topic on which the state will
...
/*
   ...
   are stored in the variables T{a,b,c} and P{a,b,c}, passed in by
...
*/
void publishReadings(float* Pa, float Ta, float* Pb, float Tb, float* Pc, float Tc) {
  ...
  for (int idx = 0; idx < NUM_MEAS; idx++) {
    ...
    mqttClient.print(idx);
    mqttClient.print(F("\" : "));
    (Pc[idx] > 80000.0 && Pc[idx] < 120000.0) ? mqttClient.print(Pc[idx] - 100000.0) : mqttClient.print(-100000.0);
  }
  mqttClient.print(F(",\n\t\"Ta"));
  mqttClient.print(F("\" : "));
  ...
  mqttClient.print(F(",\n\t\"Tc"));
  mqttClient.print(F("\" : "));
  (Tc > -40.0 && Tc < 80.0) ? mqttClient.print(Tc) : mqttClient.print("-100.0");
  mqttClient.print(F("\n}"));
  mqttClient.endMessage();
...
void checkBattery() {
  ...
  while (battVoltage > 0.1 && battVoltage < MIN_BATT_VOLTAGE) { // if something goes wrong, voltage will read 0. Allow program to proceed in this case
    ...
    mqttClient.stop();
    Serial.println(F("If connected, disconnecting from WiFi..."));
    WiFi.disconnect();
    WiFi.end();
    Serial.println(F("WiFi disconnected"));
    Serial.println(F("Shutting down ECC chip..."));
